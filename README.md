...
- IP 检测仅访问代码内固定的 HTTPS 服务，并设置短超时和响应长度限制。
...
在 MWEF 的“框架设置”中上传 `dist/mwef-app-nekocoffee-1.0.0.tar.gz`，审阅并授予所请求权限，然后启用插件。改变流量接管或 DNS 模式时，运行中的 ShellCrash 会被重启一次以载入配置。
